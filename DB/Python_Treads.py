# When you use threads it is like you are running multiple programs at once. Threads actually take turns executing.
...
def executeThread(i):
    ...
    # strftime or string formatted time allows you to define how the time is displayed. You could include the date with
...
for i in range(10):
    ...
    # Each time through the loop a Thread object is created, you pass it the function to execute and any arguments to
    # pass to that method. The arguments passed must be a sequence which is why we need the comma with 1 argument.
...
# You can lock other threads from executing. If we try to model a bank account we have to make sure the account is
# locked down during a transaction so that if more then 1 person tries to withdrawal money at once we don't give out
# more money then is in the account
...
class BankAccount (threading.Thread):
    ...
    def getMoney(customer):
        print("{} tries to withdrawal ${} at {}".format(customer.name, customer.moneyRequest, time.strftime(
            "%H:%M:%S", time.gmtime())))
